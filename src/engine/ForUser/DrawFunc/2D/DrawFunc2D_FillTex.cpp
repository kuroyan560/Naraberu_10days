...
//DrawExtendGraph
int KuroEngine::DrawFunc2D_FillTex::s_DrawExtendGraphCount = 0;

//DrawRotaGraph
int KuroEngine::DrawFunc2D_FillTex::s_DrawRotaGraphCount = 0;

static std::vector<KuroEngine::RootParam>ROOT_PARAMETER =
{
	{D3D12_DESCRIPTOR_RANGE_TYPE_CBV, "平行投影行列定数バッファ"},
	{D3D12_DESCRIPTOR_RANGE_TYPE_SRV, "元のテクスチャリソース"},
	{D3D12_DESCRIPTOR_RANGE_TYPE_SRV, "塗りつぶす用テクスチャリソース"},
};

void KuroEngine::DrawFunc2D_FillTex::DrawGraph(const Vec2<float>& LeftUpPos, const std::shared_ptr<TextureBuffer>& DestTex, const std::shared_ptr<TextureBuffer>& SrcTex, 
...
void KuroEngine::DrawFunc2D_FillTex::DrawExtendGraph2D(const Vec2<float>& LeftUpPos, const Vec2<float>& RightBottomPos, const std::shared_ptr<TextureBuffer>& DestTex, const std::shared_ptr<TextureBuffer>& SrcTex,
	const float& SrcAlpha, const Vec2<bool>& Miror, const Vec2<float>& LeftUpPaintUV, const Vec2<float>& RightBottomPaintUV)
{
...
void KuroEngine::DrawFunc2D_FillTex::DrawRotaGraph2D(const Vec2<float>& Center, const Vec2<float>& ExtRate, const float& Radian, 
