...
KuroEngine::D3D12App* KuroEngine::D3D12App::s_instance = nullptr;
std::map<std::string, KuroEngine::D3D12App::LoadLambda_t> KuroEngine::D3D12App::s_loadLambdaTable;

void KuroEngine::D3D12App::Initialize(const HWND& Hwnd, const Vec2<int>& ScreenSize, const bool& UseHDR, const Color& ClearValue, const bool& IsFullScreen)
{
//デバッグレイヤー
#ifdef _DEBUG
	//デバッグレイヤーをオンに
	...
	if (SUCCEEDED(m_device->QueryInterface(IID_PPV_ARGS(&infoQueue))))
	{
		...
		//GPU_BASED_VALIDATION_RESOURCE_STATE_IMPRECISEの抑制---------------------------------------
...
Microsoft::WRL::ComPtr<ID3D12RootSignature> KuroEngine::D3D12App::GenerateRootSignature(const std::vector<RootParam>& RootParams, std::vector<D3D12_STATIC_SAMPLER_DESC>& Samplers)
{
...
std::shared_ptr<KuroEngine::VertexBuffer> KuroEngine::D3D12App::GenerateVertexBuffer(const size_t& VertexSize, const int& VertexNum, void* InitSendData, const char* Name, const bool& RWStructuredBuff)
{
	//リソースバリア
...
std::shared_ptr<KuroEngine::IndexBuffer> KuroEngine::D3D12App::GenerateIndexBuffer(const int& IndexNum, void* InitSendData, const char* Name, const DXGI_FORMAT& IndexFormat)
{
	//リソースバリア
	auto barrier = D3D12_RESOURCE_STATE_GENERIC_READ;
...
std::shared_ptr<KuroEngine::ConstantBuffer> KuroEngine::D3D12App::GenerateConstantBuffer(const size_t& DataSize, const int& ElementNum, void* InitSendData, const char* Name)
{
	//アライメントしたサイズ
...
std::shared_ptr<KuroEngine::StructuredBuffer> KuroEngine::D3D12App::GenerateStructuredBuffer(const size_t& DataSize, const int& ElementNum, void* InitSendData, const char* Name)
{
	//リソースバリア
	auto barrier = D3D12_RESOURCE_STATE_GENERIC_READ;
...
std::shared_ptr<KuroEngine::RWStructuredBuffer> KuroEngine::D3D12App::GenerateRWStructuredBuffer(const size_t& DataSize, const int& ElementNum, void* InitSendData, const char* Name)
{
...
std::shared_ptr<KuroEngine::TextureBuffer> KuroEngine::D3D12App::GenerateTextureBuffer(const std::string& LoadImgFilePath, const bool& SRVAsCube)
{
	...
	//ワイド文字変換
	auto wtexpath = GetWideStrFromStr(LoadImgFilePath);

	//拡張子取得
	auto ext = GetExtension(LoadImgFilePath);
...
std::shared_ptr<KuroEngine::TextureBuffer> KuroEngine::D3D12App::GenerateTextureBuffer(const Vec2<int>& Size, const DXGI_FORMAT& Format, const char* Name)
{
	//テクスチャリソース設定
	...
	if (Name != nullptr)
	{
		buff->SetName(GetWideStrFromStr(Name).c_str());
	}
...
std::shared_ptr<KuroEngine::TextureBuffer> KuroEngine::D3D12App::GenerateTextureBuffer(const std::vector<char>& ImgData, const int& Channel)
{
...
void KuroEngine::D3D12App::GenerateTextureBuffer(std::shared_ptr<TextureBuffer>* Array, const std::shared_ptr<TextureBuffer>& SorceTexBuffer, const int& AllNum, const Vec2<int>& SplitNum, const std::string& Name)
{
...
std::shared_ptr<KuroEngine::IndirectCommandBuffer> KuroEngine::D3D12App::GenerateIndirectCommandBuffer(const EXCUTE_INDIRECT_TYPE& IndirectType, const int& MaxCommandCount, const int& GpuAddressNum, const bool& CounterBuffer, const void* InitCommandData, const char* Name)
{
	...
	auto hr = m_device->CreateCommittedResource(
		...
		IID_PPV_ARGS(&buff));
	assert(SUCCEEDED(hr));
	if(Name)buff->SetName(GetWideStrFromStr(Name).c_str());
	...
	//ビュー生成
	auto uavDescHandles = KuroEngine::D3D12App::Instance()->CreateUAV(buff, uavDesc, CounterBuffer ? counterBuffer->GetBuff() : nullptr);
	auto srvDescHandles = KuroEngine::D3D12App::Instance()->CreateSRV(buff, srvDesc);
...
KuroEngine::DescHandles KuroEngine::D3D12App::CreateSRV(const ComPtr<ID3D12Resource>& Buff, const D3D12_SHADER_RESOURCE_VIEW_DESC& ViewDesc)
{
...
KuroEngine::DescHandles KuroEngine::D3D12App::CreateRTV(const ComPtr<ID3D12Resource>& Buff, const D3D12_RENDER_TARGET_VIEW_DESC* ViewDesc)
{
...
KuroEngine::DescHandles KuroEngine::D3D12App::CreateDSV(const ComPtr<ID3D12Resource>& Buff, const D3D12_DEPTH_STENCIL_VIEW_DESC* ViewDesc)
{
...
KuroEngine::DescHandles KuroEngine::D3D12App::CreateUAV(const ComPtr<ID3D12Resource>& Buff, const D3D12_UNORDERED_ACCESS_VIEW_DESC& ViewDesc, const ComPtr<ID3D12Resource>& CounterBuff)
{
	m_descHeapCBV_SRV_UAV->CreateUAV(m_device, Buff, ViewDesc, CounterBuff);
	return DescHandles(m_descHeapCBV_SRV_UAV->GetCpuHandleTail(), m_descHeapCBV_SRV_UAV->GetGpuHandleTail());
}

std::shared_ptr<KuroEngine::RenderTarget> KuroEngine::D3D12App::GenerateRenderTarget(const DXGI_FORMAT& Format, const Color& ClearValue, const Vec2<int> Size, const wchar_t* TargetName, D3D12_RESOURCE_STATES InitState, int MipLevel, int ArraySize)
{
	//レンダーターゲット設定
...
std::shared_ptr<KuroEngine::DepthStencil> KuroEngine::D3D12App::GenerateDepthStencil(const Vec2<int>& Size, const DXGI_FORMAT& Format, const float& ClearValue)
{
	//リソース設定
...
void KuroEngine::D3D12App::SetDescHeaps()
{
	//ディスクリプタヒープをセット
...
void KuroEngine::D3D12App::Render(D3D12AppUser* User)
{
	SetDescHeaps();
...
void KuroEngine::D3D12App::UploadCPUResource(std::shared_ptr<GPUResource>& Dest, const size_t& DataSize, const int& ElementNum, const void* SendData)
{
	ComPtr<ID3D12GraphicsCommandList> command;
...
#include <filesystem>
#include<fstream>
Microsoft::WRL::ComPtr<IDxcBlob>KuroEngine::D3D12App::CompileShader(const std::string& FilePath, const std::string& EntryPoint, const std::string& ShaderModel)
{
	HRESULT hr;
	...
	auto w_filePath = GetWideStrFromStr(FilePath);
	auto w_entryPoint = GetWideStrFromStr(EntryPoint);
	auto w_shaderModel = GetWideStrFromStr(ShaderModel);
...
std::shared_ptr<KuroEngine::GraphicsPipeline>KuroEngine::D3D12App::GenerateGraphicsPipeline(
...
std::shared_ptr<KuroEngine::ComputePipeline> KuroEngine::D3D12App::GenerateComputePipeline(const ComPtr<IDxcBlob>& ComputeShader, const std::vector<RootParam>& RootParams, std::vector<D3D12_STATIC_SAMPLER_DESC> Samplers)
{
	HRESULT hr;
...
std::shared_ptr<KuroEngine::IndirectDevice> KuroEngine::D3D12App::GenerateIndirectDevice(const EXCUTE_INDIRECT_TYPE& ExcuteIndirectType, const std::vector<RootParam>& RootParams, std::vector<D3D12_STATIC_SAMPLER_DESC> Samplers)
{
	//Indirectの形式？
...
void KuroEngine::D3D12App::SetBackBufferRenderTarget()
{
	m_swapchain->GetBackBufferRenderTarget()->ChangeBarrier(m_commandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
