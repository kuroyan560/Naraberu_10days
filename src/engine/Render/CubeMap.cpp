...
std::shared_ptr<KuroEngine::TextureBuffer>KuroEngine::CubeMap::s_defaultCubeMapTex;

const std::array<std::string, KuroEngine::CubeMap::SURFACE_NUM>KuroEngine::CubeMap::s_surfaceNameTag =
{
	"- Front(+Z)",
	"- Back(-Z)",
	"- Right(+X)",
...
void KuroEngine::CubeMap::CopyCubeMap(std::shared_ptr<CubeMap> Src)
{
	this->m_cubeMap->CopyTexResource(D3D12App::Instance()->GetCmdList(), Src->m_cubeMap.get());
}

std::shared_ptr<KuroEngine::StaticallyCubeMap>& KuroEngine::StaticallyCubeMap::GetDefaultCubeMap()
{
...
void KuroEngine::StaticallyCubeMap::ResetMeshVertices()
{
	//頂点の順番
	...
	for (int surfaceIdx = 0; surfaceIdx < SURFACE_NUM; ++surfaceIdx)
	{
		...
		{
			//基準面の頂点を回転させて求める
			m_surfaces[surfaceIdx].m_mesh.vertices[vertIdx].m_pos = KuroEngine::Math::TransformVec3(m_surfaces[FRONT].m_mesh.vertices[vertIdx].m_pos, OFFSET_MAT[surfaceIdx]);
			//UVは同じ
...
KuroEngine::StaticallyCubeMap::StaticallyCubeMap(const std::string& Name, const float& SideLength) : m_name(Name), m_sideLength(SideLength)
{
	//デフォルトのカラー
...
void KuroEngine::StaticallyCubeMap::SetSideLength(const float& Length)
{
	m_sideLength = Length;
	ResetMeshVertices();
	for (int surfaceIdx = 0; surfaceIdx < SURFACE_NUM; ++surfaceIdx)
...
void KuroEngine::StaticallyCubeMap::AttachTex(const std::string& Dir, const std::string& Extention)
{
...
void KuroEngine::StaticallyCubeMap::Draw(Camera& Cam)
{
...
int KuroEngine::DynamicCubeMap::s_id = 0;
std::shared_ptr<KuroEngine::ConstantBuffer>KuroEngine::DynamicCubeMap::s_viewProjMat;

KuroEngine::DynamicCubeMap::DynamicCubeMap(const int& CubeMapEdge)
{
	if (s_id == 0)
	{
...
void KuroEngine::DynamicCubeMap::Clear()
{
...
void KuroEngine::DynamicCubeMap::DrawToCubeMap(LightManager& LigManager, const std::vector<std::weak_ptr<ModelObject>>& ModelObject, int Layer)
{
	...
	KuroEngine::KuroEngineDevice::Instance()->Graphics().SetRenderTargets({ m_cubeRenderTarget }, m_cubeDepth);
	KuroEngine::KuroEngineDevice::Instance()->Graphics().SetGraphicsPipeline(PIPELINE);
